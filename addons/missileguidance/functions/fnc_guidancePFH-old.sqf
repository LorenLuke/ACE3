...
if (!((count (_timeParams)) > 0) ) then {
	_timeParams pushBack 0; //0- lastFrameTime
    _timeParams pushBack 0: //0- last time
	_timeParams pushBack 0; //1- time delta
	_timeParams pushBack 0; //2- time modulus
	_timeParams pushBack 0; //3- time modulus threshold
};
_timeParams params ["_lastTime", "_lastTime", "_timeDelta", "_timeModulus", "_timeModulusThreshold"];
