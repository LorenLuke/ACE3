...
{
    private _xPos = _x modelToWorldVisualWorld [0,0,0];
...
//        hint format ["%1", [_pos, _xPos, true, _object, _x]];
    };
    if (_angleOkay && _losOkay && isEngineOn _x && (_x != _shooter)) then {
        _targetsList pushBack _x;
        drawIcon3D ["\a3\ui_f\data\IGUI\Cfg\Cursors\selectover_ca.paa", [1,0,1,1], ASLtoAGL _xPos, 0.75, 0.75, 0, str(_x), 1, 0.025, "TahomaB"];
    } else {
        drawIcon3D ["\a3\ui_f\data\IGUI\Cfg\Cursors\selectover_ca.paa", [1,0,0,1], ASLtoAGL _xPos, 0.75, 0.75, 0, str(_x), 1, 0.025, "TahomaB"];
    };
...

{
    ...
    } forEach ((getPos _y) nearObjects ["CMflareAmmo", 75]);
} forEach _targetsList + [_object];
...
private _foundTargetPos = (_seekerTargetsList select 0) modelToWorldVisualWorld ((_seekerTargetsList select 0) selectionPosition ["HitEngine","HitPoints"]);
